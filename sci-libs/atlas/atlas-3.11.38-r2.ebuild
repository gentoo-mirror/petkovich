...
IUSE="doc fortran generic ifko lapack static-libs threads hardfp"
...
src_configure() {
	...
	atlas_configure() {
      ...
      myconf+=( "-b 64" )
    elif use arm && use hardfp; then
        myconf+=( "-b 32 -D c -DATL_ARM_HARDFP=1 -Si archdef 0 -Fa alg -mfloat-abi=hard" )
    fi
		if use fortran; then
			myconf+=(
